...
const Identification = ({nom, adresse, ville, gerant, ape}) => {
	...
	<article className="identite" onClick={ouvertureFermetureGlobale}>
		...
		<dl>
		    ...
		    <dd className="naissance">{gerant.civilite === "Monsieur" ? "Né" :"Née"} le {gerant.naissance.toLocaleString('fr-FR', {day:'numeric', month:'long', year:'numeric'})} {gerant.villeNaissance.a === null ? `à ${gerant.villeNaissance.nom}` : `̀${gerant.villeNaissance.a}`}, {gerant.villeNaissance.nomDepartement}.</dd>
		    <dd className="gerant">S.S. {gerant.ss}</dd>
